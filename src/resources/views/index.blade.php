...
@section('content')
    <div class="index_edge">
        <p class="greeting">
            {{ str_replace([' ', '　'], '', $user->name) }}さん&nbsp;お疲れ様です！
        </p>
        @if (session('action'))
        ...
        <div class="attendance-records">
            ...
            <div class="record_item">
                <form action="{{ route('work.end') }}" method="POST">
                    ...
                    @else
                        <button class="button end_work attendance-button" type="submit" name="end_work"
            ...
            <div class="record_item">
                <form action="{{ route('break.start') }}" method="POST">
                    @csrf
                    @if ($status == 1)
                        <button class="button break_records attendance-button" type="submit"
                            name="start_break">休憩開始</button>
                    @else
                        <button class="button break_records attendance-button" type="submit" name="start_break"
            ...
            <div class="record_item">
                <form action="{{ route('break.end') }}" method="POST">
                    ...
                    @else
                        <button class="button end_break attendance-button" type="submit" name="end_break"
